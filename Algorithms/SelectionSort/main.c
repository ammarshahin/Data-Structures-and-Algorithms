...
/* set the maximum value to the first element */
        ...
        c3=0;
		for(i=0;i<=j;i++){ // loop through the elements to cheek if there was a bigger value than max
			if(arr[i] > max){
				max = arr[i]; // change the the max value to the new max
				index = i;   // change the the max index to the new max index
			...
			}else if(arr[i] == max){ // cheek whether there are elements equals the max to be counted with C1 and C3
				c1++;
                c3++;
			}
		}
